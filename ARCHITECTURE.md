...
- **Navigation**: Microverse (brand/landing) · Dashboard (task list) · Reports (placeholder — PM-facing throughput, kudos leaderboard, analyst efficiency; not designed in detail yet) · **Projects** (PM-facing, own subnav: Projects, Accounts — in business terms, an Account Management/Client Success hub for upsell insights and SLA tracking) · **Admin** (`platform:admin` only, own subnav: Users, Services). Services admin capabilities: activate/deactivate a service platform-wide, add a new service on release, edit a service's card details. **Avatar (top-right)** opens a dropdown with My Profile (4.0.4) and Log out — the standard, expected spot for both, not a separate nav item.
