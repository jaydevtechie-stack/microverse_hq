...
|---|---|---|---|
| elixtempo | Elixir | The trust layer between "work happened" and "money/accountability follows" — tracked time feeds customer billing, analyst payouts, and business-efficiency reporting alike (see [docs/business/1.0/product-strategy.md](../../business/1.0/product-strategy.md)'s product definition). OTP concurrency handles many cheap live sessions. Also the input rustledger's payout side needs — see that service's note in [docs/architecture/1.0/domain-services.md](../1.0/domain-services.md). | Online — session lifecycle, caller-identity auth, Postgres-backed crash recovery (live-verified restart), an hours-aggregate query, `quest_id` validation against task-service, and a TaskFusion timer widget (live-verified in browser) all built; live-consumed by RustLedger for billing. All 5 planned phases done — see [docs/roadmap/1.1/domain-services.md](../../roadmap/1.1/domain-services.md) |
...
| PyReel | Python | Video sentiment/quality analysis for customer-submitted call recordings and video interactions — the video counterpart to GoFeeler's text-sentiment thesis, piping transcription into GoFeeler's own `/analyze` rather than duplicating analysis logic. | Phase 1 done, live-verified — real `ffprobe` metadata (duration/resolution/codec/has_audio) via sync and Kafka-async endpoints, no fake data. No transcription/task-pool wiring/frontend yet (Phases 2-4) — see [docs/roadmap/1.1/domain-services.md](../../roadmap/1.1/domain-services.md) |
