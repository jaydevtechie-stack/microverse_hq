...
|---|---|---|---|
| elixtempo | Elixir | The trust layer between "work happened" and "money/accountability follows" — tracked time feeds customer billing, analyst payouts, and business-efficiency reporting alike (see [docs/business/1.0/product-strategy.md](../../business/1.0/product-strategy.md)'s product definition). OTP concurrency handles many cheap live sessions. Also the input rustledger's payout side needs — see that service's note in [docs/architecture/1.0/domain-services.md](../1.0/domain-services.md). | Designing |
