...
**Branches map to roadmap items, not the other way around.** Each branch corresponds to a phase/branch entry in [docs/roadmap/](../roadmap/) (e.g. "Branch 4.1", "Phase 10"). Default to one branch per roadmap item; split off a dedicated branch for a sub-item (e.g. 4.1.1) only when it's substantial enough to deserve its own review cycle separate from its parent item — small sub-items ride along in the parent branch instead. Either way, the branch's slug or its PR/first commit should make the roadmap number unambiguous, so `git log`/PR history can be traced straight back to a roadmap entry.
...
- When a commit implements a specific roadmap item, reference it in parentheses at the end, matching the numbering in [docs/roadmap/](../roadmap/) — e.g. `feat(taskfusion): real Project Hub + Admin pages, roles applied (4.0.3)`. This is what makes it possible to trace "when did Branch 4.1 actually land" straight from `git log`.

## Docs stay in sync with code

This repo treats [docs/architecture/](../architecture/), [docs/roadmap/](../roadmap/), and [docs/schema.md](../schema.md) as living documents, not a one-time spec:
...
- If a PR changes a real, migrated table shape, update [docs/schema.md](../schema.md) — that file explicitly defers to the real migrations as source of truth, but only stays useful if it's kept honest.
- If a PR closes a security gap listed in [docs/security.md](../security.md)'s "Known gaps," remove it from there too.
...
- One topic branch per roadmap item/sub-item where practical, so a PR maps cleanly to one entry in [docs/roadmap/](../roadmap/).
