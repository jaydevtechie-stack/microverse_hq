...
- 🟡 5.1 — `SentimentEngine` interface (`basic` | `advanced`) in the Go service. `basic` is the existing keyword matcher, refactored behind the interface with no behavior change. `advanced` is new: calls out to an LLM via the `Provider` interface (see 5.1.1). Every result stamps `engine_used`.
- 🟡 5.1.1 — `Provider` interface underneath `advanced` (`Complete(prompt, opts) (Result, error)`), GoFeeler-local for now. This is the plug-and-play seam: a new LLM provider is a new implementation of this one interface, registered by config — no change to engine logic. Deliberately scoped as a GoFeeler-internal package, not a standalone gateway service, per the "introduce as a service when a second consumer appears" threshold — GoFeeler-advanced is the first consumer. When `intelligence/ai-tools` ([docs/architecture/2.0/intelligence.md](../../architecture/2.0/intelligence.md)) exists, it plugs in as just another `Provider` implementation, not a rewrite. Revisit consolidating this into a shared library once a second polyglot service wants LLM capability — parked as a Phase 3.0+ item, not blocking Branch 5.
- 🟡 5.2 — `sentiment_prompt_templates` table (Postgres): shared pool, visible to all analysts, any analyst can create/edit (self-service, no gating — blast radius is contained to their own analyses). System ships a small set of preconfigured templates (`is_system_default`). Distinct from `intelligence/prompts` ([docs/architecture/2.0/intelligence.md](../../architecture/2.0/intelligence.md)) — that folder is version-controlled, dev-curated prompt text for agent reasoning; this table is a runtime, analyst-authored, UI-editable library scoped to GoFeeler specifically. Different audience, different mechanism, same word.
- 🟡 5.3 — Request/response contract: request carries `engine: "basic" | "advanced"` and optional `templateId` (advanced only, falls back to system default). Response always stamps `engine_used`, `template_id`, `llm_provider`, `model_version` — see [docs/schema.md](../../schema.md) for where these land in the `gofeeler` Mongo result shape and how they flow downstream to Djaboard.
- 🟡 5.4 — Governance (see [docs/security.md](../../security.md)): customer-submitted text is untrusted input once interpolated into an LLM prompt — prompt-injection surface, flagged explicitly rather than assumed away. Advanced-engine calls are the first place Microverse spends real external-API money per request.
- 🟡 5.5 — Reproducibility note: advanced-engine results are not directly comparable across requests unless `template_id` (and `llm_provider`/`model_version`) match — stated plainly wherever sentiment results get consumed downstream, so two advanced-mode scores aren't treated as apples-to-apples without checking.
