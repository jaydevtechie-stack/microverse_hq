...
- **No real ownership check on upload** — `order_id` is task-service's own `tasks.id`; there's no separate Order entity or order-service (see [docs/architecture/1.0/business-services.md](../../architecture/1.0/business-services.md)), so this isn't blocked on a future service. asset-service already calls `task_client::fetch_status` for download/list; a real ownership check just means that same call also returning `customer_id` and comparing it against the caller — not implemented yet, just not yet wired up. No `company_id` either (there's no company entity anywhere in the stack, just Keycloak users) — `username` stands in for it in the object key.
- **List/download resolve the object by scanning**, not a direct key lookup — `username` sits before `order_id` in the key, and there's no way to know a customer's username from the order_id alone without a task-service round trip (the same `task_client::fetch_status` call already made for status could return it, this just hasn't been wired up). `ListObjectsV2` under the service prefix, filtered in memory for `/{order_id}/`. Fine at current scale; would need a real index or a reordered key to stay a cheap prefix lookup once object counts grow.
