...
- **Phase 2 — Crash recovery.** Resolved on Postgres write-behind (see Open questions above), broken down:
  ...
  - 🟡 **2.3 — A real kill/restart verification** against the live docker-compose stack (start a session, kill the container, restart it, confirm the session and its elapsed time are still there) — not just the unit-level rehydrate coverage 2.2 already added.
