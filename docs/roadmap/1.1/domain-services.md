...
- **Upload path — assumed, not yet confirmed. Only a Phase 2 dependency, not Phase 1.** Working assumption is PyReel eventually reuses asset-service's existing MinIO upload flow rather than inventing a second one, which needs `video/*` added to the content-type allowlist enforced in [infrastructure/nginx/conf.d/assets.conf](../../../infrastructure/nginx/conf.d/assets.conf) (currently `text/`, `image/`, `application/json`, `application/pdf` only). Doesn't block Phase 1 — see that phase's wire contract below, which sidesteps this entirely by fetching from a URL. Small, but a genuine cross-service dependency once Phase 2 actually needs real customer uploads — revisit then if a different upload shape turns out to be needed.
...
- 🟢 **Phase 1 — Basic engine, real video handling.** Real metadata via ffprobe (duration, resolution, codec, has_audio) replacing the fake `random.uniform()` stub and dropping the fake `detected_objects` field entirely — it implies object-detection ML that was never real and isn't in scope here. Async via Kafka, reusing the existing broker task-service already runs (dropping the current pika/RabbitMQ stub, which was never deployed) rather than standing up a second message broker — same "don't add infra for one gap" reasoning [business-services/workflow](../../../business-services/workflow/README.md) used to justify staying inside task-service's existing cron pattern instead of standing up a first-ever Java service.
  - **Wire contract, concrete enough to build against:** consumes `pyreel.analyze-requests` (`{request_id, video_url}` — reuses `VideoAnalysisRequest.video_url`, already defined but unused today; PyReel fetches the video over HTTP itself), publishes `pyreel.analyses` (`{request_id, duration_seconds, resolution, codec, has_audio}`) — producer-owns-its-topic naming, matching `task-service.tasks`/`elixtempo.sessions`. Deliberately URL-based, not an object-key/MinIO reference — sidesteps the Upload path open question above entirely, since Phase 1 has no real caller yet. Phase 2 is what actually has to resolve how a customer's video becomes a reachable `video_url` (or replaces this contract with the object-key list shape directly).
  - Proves real video handling and event wiring end to end before any transcription-model decision is needed.
