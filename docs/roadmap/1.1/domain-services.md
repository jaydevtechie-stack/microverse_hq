...
- **Phase 2 — Crash recovery.** Resolved on Postgres write-behind (see Open questions above), broken down:
  ...
  - 🟡 **2.2 — Rehydrate on boot.** Read every non-`stopped` row on startup and spawn a `Session` GenServer per row, seeded directly from the persisted `status`/`accumulated_seconds`/`running_since` — a crash should be transparent to the analyst's clock, not something that resets or freezes it.
  - ⚪ **2.3 — Tests + a real kill/restart verification** against the live docker-compose stack (start a session, kill the container, restart it, confirm the session and its elapsed time are still there) — not just unit tests of the rehydrate function in isolation.
- ⚪ **Phase 3 — Historical/aggregate query surface.** Very likely lands on ElixTempo now that 2.1 gave it a real table (see Open questions above) — a `GET /hours`-style endpoint reading `elixtempo.sessions` directly, no new persistence needed. This is what actually unblocks the Payouts section's "payout basis" question below, not a nice-to-have.
