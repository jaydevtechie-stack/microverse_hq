...
1. **`/api/users` has no role check, and compounds with the JWT gap above.** `business-services/task-service/routes/user-routes.js:13-66` — `GET /users`, `GET /users/:id`, and `PATCH /users/:id { active }` check nothing at all, unlike every sibling route file in the same directory (`service-routes.js` gates writes behind `requireRealmRole('platform:admin')`). Any authenticated caller, even a low-privilege one, can enumerate every user or deactivate any account including admins — and once the JWT-signature gap above is fixed, this bug is still exploitable on its own by anyone with a legitimately-issued low-privilege token. Fixing JWT verification alone does not close this. *A01 Broken Access Control, High.*
2. **Task assignment has no caller-role check.** `business-services/task-service/routes/task-routes.js:183-215` — `PATCH /tasks/:id` validates the *assignee* holds `platform:analyst` + the right `service:*` role but never checks the *caller* is a PM, despite the route's own comment describing it as PM-only. Any authenticated user can hijack the assignment flow and corrupt task-status invariants. *A01, High.*
...
4. **asset-service's `list_assets` skips the ownership check its two sibling endpoints enforce.** `platform-services/asset-service/src/api.rs:217-247` — `download_url` and `delete_asset` both check the caller owns the order (`is_customer && !matching_key.contains(username)`); `list_assets` only checks the `service:*` role. Tempered by `order_id` being an unguessable `crypto.randomUUID()`, but a real inconsistency between three near-identical handlers. *A01, Medium.*
5. **rustledger has no authentication on any endpoint.** `domain-services/rustledger/src/api.rs:17-81` — unlike every other service, it doesn't even do the (currently-unverified) JWT decode. Currently reachable only Docker-internally (no nginx route exists), but zero auth on full billing data. *A01, Medium.*
...
9. **`ws` dependency (notification-service) has a real uninitialized-memory-disclosure CVE**, not just the usual DoS-class advisory noise `npm audit` otherwise turns up across the Node services — worth prioritizing this one upgrade specifically. *A06 Vulnerable and Outdated Components, Low-Medium.*
