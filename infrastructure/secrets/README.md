...
openssl req -x509 -nodes -newkey rsa:2048 -days 825 \
  ...
  -addext "subjectAltName=DNS:microverse.local,DNS:sso.microverse.local,DNS:dashboard.microverse.local,DNS:localhost"
```

Self-signed, so browsers will warn on first visit — expected for local
dev. Map `microverse.local`, `sso.microverse.local`, and
`dashboard.microverse.local` to `127.0.0.1` in your hosts file.
